...
import AutocompleteQuerystringSchema from 'schema/assets/autocopmlete/querystring.json'
import GetTickerParamsSchema from 'schema/assets/getTicker/param.json'
  ...
  fastify.get<{ Querystring: AutocompleteQuerystring }>(
    ...
    (request, reply) => {
      const results = fastify.searchEngine
        ...
        .map(
          (result: {
            item: { id: any; ticker: any; name: any; image: any }
          }) => ({
            id: result.item.id,
